...
export default function JohnWindwardProfile({ agentMetrics, posts }) {
  ...
  useEffect(() => {
    ...
    const metrics = [
      ...
      { label: 'Anomalous recv', jw: jw.anomaly_recv_total,  avg: avg.anomaly_recv / avg.count }
    ]

    const svg = d3.select(barRef.current)
    svg.selectAll('*').remove()

    const W = 320, H = metrics.length * 60 + 50
    ...
    const yScale = d3.scaleBand().domain(metrics.map(d => d.label)).range([0, H - m.top - m.bottom]).padding(0.3)
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap gap-4">
        {/* Profile card */}
        <div className="bg-slate-800 border border-red-900/40 rounded-lg p-5 w-64 shrink-0">
          ...
          {[
            ...
            ['Normal recv', jw.recv_normal, '#64748b'],
            ['Anomalous recv', jw.anomaly_recv_total, '#e63946'],
            ['Incidents', jw.incidents_involved?.join(', '), '#f4a261'],
            ['Is C2 agent', jw.is_c2_agent ? 'No' : 'No', '#64748b']
          ].map(([k, v, color]) => (
            <div key={k} className="flex justify-between text-xs py-1 border-b border-slate-700/40">
              <span className="text-slate-500">{k}</span>
              <span className="font-semibold text-right max-w-[120px]" style={{ color }}>{String(v)}</span>
            </div>
          ))}
        ...
        <div className="bg-slate-900/60 rounded-lg border border-slate-700 p-4 flex-shrink-0">
          ...
          <div className="flex gap-4 mt-2 text-xs text-slate-500">
            ...
            <span className="flex items-center gap-1.5"><span className="w-3 h-2 bg-slate-600 inline-block rounded" />System avg</span>
          </div>
        </div>
      </div>

      {/* Post timeline */}
      <div className="bg-slate-900/60 rounded-lg border border-slate-700 p-4" ref={timelineWrapRef}>
        <div className="text-sm font-semibold text-slate-300 mb-2">SaidIT Posts — All 108 posts, 3 anomalous (john_windward)</div>
