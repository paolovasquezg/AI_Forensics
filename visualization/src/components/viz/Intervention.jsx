...
export default function InterventionRecommender({ interventionEdges }) {
  ...
  useEffect(() => {
    ...
    rowG.append('rect')
      ...
      .on('mouseover', function () { d3.select(this).attr('fill', 'rgba(43,40,35,0.06)') })
    ...
    rowG.append('text')
      ...
      .attr('fill', (d) => selected === d ? '#2b2823' : '#4d4842')
      .attr('font-size', 10)
  ...
  return (
    <div className="space-y-5">
      ...
      {best && (
        <div className="bg-slate-100 border border-red-200 rounded-lg p-5 border-l-2 border-l-red-500">
          <div className="text-xs text-slate-500 uppercase tracking-widest mb-2">Top recommendation</div>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="text-lg font-bold text-slate-900">
                <span className="text-red-600">{agentLabel(best.from)}</span>
                <span className="text-slate-500 mx-2">→</span>
                <span className="text-red-600">{agentLabel(best.to)}</span>
              </div>
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                ...
                {best.normal_count === 0 && (
                  <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-50 text-green-600 border border-green-300">
            ...
            <div className="flex gap-6 text-center shrink-0">
              <div>
                <div className="text-2xl font-bold text-red-600">{best.total_anomalous}</div>
                <div className="text-xs text-slate-500">Anomalous uses</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-slate-800">{best.incidents_count}</div>
                <div className="text-xs text-slate-500">Incidents covered</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-slate-800">{best.normal_count}</div>
      ...
      {/* Stacked bar chart */}
      <div ref={wrapRef} className="bg-white rounded-lg border border-slate-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="text-base font-semibold text-slate-700">Top 10 candidate edges</div>
          <div className="flex gap-3">
            {INCIDENTS.map(inc => (
              <span key={inc} className="flex items-center gap-1.5 text-xs text-slate-600">
      ...
      {detail && (
        <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="rounded p-2 text-center bg-slate-100 border border-slate-200">
              <div className={`text-lg font-bold ${detail.normal_count === 0 ? 'text-green-600' : 'text-amber-600'}`}>
                {detail.normal_count}
              </div>
          ...
          <p className="text-xs text-slate-600 border-t border-slate-200 pt-3">
            {detail.normal_count === 0
              ? <span className="text-green-600 font-medium">Blocking this edge disrupts zero normal operations</span>
              : <span className="text-amber-600 font-medium">Blocking this edge would disrupt {detail.normal_count} normal delegation(s) </span>
            }
          </p>
        </div>
      )}
